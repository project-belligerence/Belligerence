(function() {
	...
	var PMCModel = require('./../index.js').getModels().pmc,
		...
		FriendsMethods = require('./../index.js').getMethods().friends,
		GeneralMethods = require('./../index.js').getMethods().general_methods,
		ActionsCostMethods = require('./../index.js').getMethods().actions_cost,
	...
	function initializeWebsocketEvents() {
		...
		config.websocket.registerEvent("NewInvite", WebsocketEvent);
	}
		...
		switch(invite) {
			case 'Request_PlayerPMC': {
				switch(type) {
					...
					case ("verbose"): { object.value = "Player request to join an Outfit."; } break;
					case ("params"): { object.value = [req.playerInfo.hashField, [!req.playerInfo.PMCId, 'You are already part of an Outfit.'], req.body.point_b]; } break;
					...
					case ("results"): {
						var blockInvites = (_.indexOf(oB.privateFields, 'blockInvites') > -1),
							canRead = PMCMethods.getVisibility(req, (oB.privateVisibility || "nobody"), oB);
						object.value = [oA, oB, [oB.openForApplications, config.messages().modules.pmc.not_open_applications], [(!(blockInvites) && canRead), config.messages().bad_permission]];
					} break;
					...
					case ("sent"): { config.websocket.broadcastEvent("NewInvite", ["pmc", oB.hashField]); } break;
					case ("resolve"): {
			...
			case 'Invite_PlayerPMC': {
				switch(type) {
					...
					case ("data"): { object.value = {a: "Pm", b: "Pl"}; } break;
					case ("cost"): { object.value = __globalPrices.costInvitesPlayer; } break;
					case ("verbose"): { object.value = "Outfit invitation for a player to join their ranks."; } break;
					...
					case ("results"): {
						var blockInvites = (_.indexOf(oB.privateFields, 'blockInvites') > -1),
							canRead = PlayerMethods.getVisibility(req, (oB.privateVisibility || "nobody"), oB);
						object.value = [oA, oB, [!(oB ? oB.PMCId : null), config.messages().modules.pmc.that_in_pmc], [(!(blockInvites) || canRead), config.messages().bad_permission]]; } break;
					...
					case ("sent"): { config.websocket.broadcastEvent("NewInvite", ["player", oB.hashField]); } break;
					case ("resolve"): {
			...
			case 'Friends_Player': {
				switch(type) {
					...
					case ("results"): {
						var blockInvites = (_.indexOf(oB.privateFields, 'blockInvites') > -1),
							canRead = PlayerMethods.getVisibility(req, (oB.privateVisibility || "nobody"), oB);
						object.value = [oA, oB, [!oC, config.messages().modules.friends.already_friends], [!oD, config.messages().modules.friends.already_friends], [(!(blockInvites) || canRead), config.messages().bad_permission]];
					} break;
					case ("query"): { object.value = {where: {'point_a': [req.playerInfo.hashField, req.body.point_b], 'point_b': [req.body.point_b, req.playerInfo.hashField]}}; } break;
			...
			case 'Friends_PMC': {
				switch(type) {
					...
					case ("cost"): { object.value = __globalPrices.costInvitesPMC; } break;
					case ("verbose"): { object.value = "An alliance offer from an Outfit to another."; } break;
					...
					case ("results"): {
						var blockInvites = (_.indexOf(oB.privateFields, 'blockInvites') > -1),
							canRead = PMCMethods.getVisibility(req, (oB.privateVisibility || "nobody"), oB);
						object.value = [oA, oB, [!oC, config.messages().modules.friends.already_friends_pmc], [!oD, config.messages().modules.friends.already_friends_pmc], [(!(blockInvites) || canRead), config.messages().bad_permission]];
					} break;
					case ("query"): { object.value = {where: {'point_a': [req.playerInfo.PMC.hashField, req.body.point_b], 'point_b': [req.body.point_b, req.playerInfo.PMC.hashField]}}; } break;
		...
		mainModel.findAll(query.value).then(function(entries) {
			...
			FriendsMethods.getFriendsAllFunc(req, res, function(friendsList) {
				...
				models.pointD.findOne({where: targets.value.point_d}).then(function(object_D) {
					...
					ActionsCostMethods.getActiveFunc(req, res, function(costs) {
						__globalPrices = costs.dataValues;
						...
						GeneralMethods.spendFundsGeneralFunc(req, res, handleInvite(req, res, req.body.type, "cost").value, function(done) {
							...
							mainModel.sync({force: false}).then(function() {
								mainModel.create(update).then(function(new_entry) {
									handleInvite(req, res, req.body.type, "sent", [], object_A, object_B, object_C, object_D);

									API.methods.sendResponse(req, res, true, config.messages().modules.invites.invite_created, new_entry);
								});
							});
						});
					});
